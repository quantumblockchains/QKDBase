...
describe('Topelitz service', () => {
    ...
    describe('checkIfToeplitzAsStringExists', () => {
        beforeEach(() => {
            toeplitzService.clearToeplitzMatrixesMapping();
        });
        test('If mapping is empty return value should be undefined', () => {
            expect(toeplitzService.checkIfToeplitzAsStringExists('address1')).toBeUndefined();    
        });
        test('Check if a given node has an assigned toeplitz matrix stored in ToeplitzMatrixMapping', () => {
            const binaryString = generateRandomBinaryArray(11);
            const toelpitzMatrix = matrixMathService().generateToeplitzMatrix(binaryString);
            toeplitzService.addToeplitzMatrix(toelpitzMatrix, 'address1'); 
            expect(toeplitzService.checkIfToeplitzAsStringExists('address1')).toEqual(toelpitzMatrix);    
        });
    });
    ...
    describe('generateToeplitzGroupSignature', () => {
        ...
        test('Generate Signature for single char', () => {
            ...
            expect(toeplitzGS).toStrictEqual(["1011111"]);
        });
